...
class DOCXtoHTML {
    ...
    function Init(){
        if($this->extractRelXML()==false){
            $this->DeleteTemps();
            $this->error = "The file data could not be found or read.";
            return false;
        }
        if($this->extractMedia()==false){
            $this->DeleteTemps();
            $this->error = "The Media could not be found.";
            return false;
        }
        ...
        if($this->DeleteTemps()==false){
            $this->error = "The temporary files created during the process could not be deleted.
    ...
    function getMediaFolder(){
        if(empty($this->content_folder)){
            ...
            $this->mediaDir = ABSPATH . "/wp-content/uploads/media/".$MediaFolder;
        } else {
            $this->mediaDir = ABSPATH . "/wp-content/uploads/media/".$this->content_folder;
        }
                ...
                switch ($value['tag']) {
                    ...
                    case "A:BLIP"://the image data
                        if($value['type'] == "open" || $value['type'] == "complete"){
                            ...
                            $imagepath = str_ireplace(ABSPATH, '', $this->rels[$rid][1]);
                            if(array_key_exists(2,$this->rels[$rid])){
                                $imagebigpath = str_ireplace(ABSPATH, '', $this->rels[$rid][2]);
                            } else {
                                $imagebigpath = false;
                            }
